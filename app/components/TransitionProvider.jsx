...
const TransitionProvider = ({children}) => {
    ...
    <div key={pathName} className="w-screen h-screen">
        <motion.div className="h-screen w-screen fixed bg-black z-40" initial="false"  animate={{height:"0vh"}} exit={{height:"140vh"}} transition={{duration:0.5, ease:"easeOut"}} />
        <motion.div className="fixed m-auto top-0 left-0 right-0 bottom-0 text-white text-8xl cursor-default z-50 w-fit h-fit" initial={{opacity:1}} animate={{opacity:0}} exit={{opacity:0}} transition={{duration:0.8, ease:"easeOut"}}>
        {pathName.substring(1)}
          ...
          <motion.div   
            ...
            transition={{ ease: "easeOut", duration: 0.75 }} className="h-[calc(100vh-6rem)]" >
            {children}
          </motion.div>
        </div>
    </AnimatePresence>
